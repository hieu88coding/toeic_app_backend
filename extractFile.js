...
async function unzipAndUploadToFirebase(url, type, testName) {
    try {
        ...
        for (const [fileName, file] of Object.entries(zipFiles.files)) {
            if (!file.dir) {
                ...
                const firebaseFilePath = `${type}/${testName}/${fileName}`;

                // Tải lên tệp tin con lên Firebase Storage
                await bucket.file(firebaseFilePath).save(fileData);
            }
        }
        const folderUrl = `https://console.firebase.google.com/project/hieu88toeicapp/storage/hieu88toeicapp.appspot.com/files/~2F${type}~2F${testName}`;
