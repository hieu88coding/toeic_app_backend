...
puppeteer.launch({ headless: false }).then(async browser => {
    async function runMultipleTests(numTests) {
        for (let i = 2; i <= numTests; i++) {
            let audioFolderPath = `D:/ToeicApp/audio/Part3/Test${i}`;
            let page = await browser.newPage()
            let navigationPromise = page.waitForNavigation()
            await page.goto(`https://estudyme.com/study/toeic-testpro/part-3-conversations/test-${i}/`)
            ...
            for (let i = 0; i < 13; i++) {
                ...
                let audioName = `part3_test${i}_audio_${i + 1}_${i + 3}.mp3`;
            ...
            for (let i = 0; i < 39; i++) {
                const row = worksheet.addRow({
                    ...
                    explanation: '',
                    transcript: transcript[Math.floor(i / 3)]
                });
            }
            //Save workbook to Excel file
            await workbook.xlsx.writeFile(`D:/ToeicApp/answer/Part3/Part3_Test${i}_answer.xlsx`);
            ...
            //Save workbook to Excel file
            await newWorkbook.xlsx.writeFile(`D:/ToeicApp/Exel/Part3/Part3_Test${i}_question.xlsx`);
        }
